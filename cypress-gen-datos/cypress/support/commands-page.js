...
Cypress.Commands.add("createPageLink", (title, stage) => {
	...
	cy.log(
		`Creando link de pagina con titulo ${title}  y link to ${titleToLink}`
	);
	...
	cy.get("#settings-navigation  .gh-blognav-line  .gh-blognav-url")
		.last()
		.find("input.ember-text-field")
		.type(titleToLink);
	// da clic en guardar
...
Cypress.Commands.add("deletePageLinkByTitle", (title, stage) => {
	...
	cy.log(
		`Eliminando link de pagina con titulo ${title}  y link to ${titleToLink}`
	);
...
Cypress.Commands.add("deletePageByTitle", (title, stage) => {
	...
	cy.get(
		"li.gh-posts-list-item>a.gh-post-list-title>h3.gh-content-entry-title"
	)
		.contains(title)
...
Cypress.Commands.add(
	"validatePageByTitleAndLink",
	(title, ocurrencias, stage) => {
		let titleToLink = title.replace(/\s/gi, "-").toLowerCase();
		cy.log(
			`Validando que la pagina con titulo ${title}  y  ${titleToLink} este publicada`
		);
		const url = Cypress.config("baseUrl");
		cy.visit(url);
		cy.screenshot(`${stage}/validate-page`);
		cy.get(
			'div.gh-head-menu>.nav>li>a[href$="/' + titleToLink + '/"]'
...
Cypress.Commands.add("filterPublishPage", (stage) => {
	cy.wait(100);
	cy.get(
		".view-actions .gh-contentfilter .gh-contentfilter-type .ember-basic-dropdown-trigger"
	).click();
