...
When('I click in save filter button', async () => {
	let btnTag = await this.driver.$(`.view-actions > .gh-contentfilter > .dropdown > .gh-btn-save-view`);
	await btnTag.click();
});
...
When('I click in save popUp button', async () => {
	let btnTag = await this.driver.$(`.modal-content > .modal-footer > .ember-view`);
	await btnTag.click();
});
